...
def generateSkillsReport(data):
	...
	df["Weak"] = df["coverage"].apply(lambda x: "✔" if x == "Weak" else "")
	df["Medium"] = df["coverage"].apply(lambda x: "✔" if x == "Medium" else "")
	df["Strong"] = df["coverage"].apply(lambda x: "✔" if x == "Strong" else "")
...
def render_lessons(lesson_data):
	...
	if lesson_data == None:
		return '🤖 <i>Error fetching lesson-level data.</i>'
