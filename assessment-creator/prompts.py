...
- **Question Type Distribution**: When "SHORT_ANSWER" is among the allowed types, use it sparingly — only for concepts that are best assessed by a free-form written explanation rather than a choice or code task. Aim for roughly **20%** of the generated questions to be SHORT_ANSWER (e.g. ~1-2 per 10 questions), and never more than ~30%. Prefer CODING or choice questions when the concept allows it. If SHORT_ANSWER is not in the allowed types, ignore this and use only the other types. When "CODING" is among the allowed types, strongly prefer CODING: if the concept being taught can be assessed by one of the available coding languages ({coding_languages}), strongly consider a CODING question. Aim for roughly **40-50%** of the generated questions to be CODING, and do not produce fewer than ~30% CODING when CODING is an allowed type.
