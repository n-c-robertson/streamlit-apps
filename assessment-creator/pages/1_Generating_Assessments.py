...
BUILD_TAG = '2026-06-19-construction-fallback-restored'
    ...
    if st.session_state.generated_questions_df is not None and not st.session_state.generated_questions_df.empty:
        ...
        if 'debug_outputs' in progress_data:
            with st.expander("🔍 Debug: Step-by-Step Outputs", expanded=False):
                st.markdown("**This shows the output generated at each step of the assessment generation pipeline.**")
                st.markdown("Use this to identify where messy data might be generated by AI vs where you can fetch values from the content object.")
                st.markdown("---")
                
                debug_outputs = progress_data['debug_outputs']
                for step_name, step_data in debug_outputs.items():
                    st.markdown(f"#### {step_name}")
                    st.write(f"**Type:** `{step_data.get('type', 'Unknown')}`")
                    st.write(f"**Output:** {step_data.get('output', 'No output captured')}")
