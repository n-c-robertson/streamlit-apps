...
# SKILLS_API_URL and JWT_TOKEN are now imported from settings

def get_skills_recommendations(user_skills_df, results_df):
    """
    ...
    # Create progress bar
    total_users = len(user_skills_df)
    progress_bar = st.progress(0)
    status_text = st.empty()
    
    for idx, (_, row) in enumerate(user_skills_df.iterrows()):
        user_id = row['userId']
        
        # Use net skills and filter for values 0 or less
        ...
        status_text.text(f"Processing user {idx + 1} of {total_users}: {user_id}")
        
        if not skills_needing_improvement:
            continue
            
        # Get user's total score
        user_total_score = row['totalScore']
        
        # Prepare payload for Skills API
        payload = {
            "search": ','.join(skills_needing_improvement),
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in api_response:
                    if item.get('search') and item['search'].get('metadata'):
                        ...
                        recommendation = {
                            ...
                            'weakSkills': skills_needing_improvement,  # Keep field name for compatibility
                ...
                st.warning(f"API request failed for user {user_id}: {response.status_code}")
                
        except Exception as e:
            st.warning(f"Error getting recommendations for user {user_id}: {str(e)}")
...
def plot_recommendation_charts(recommendations_df):
    ...
    if recommendations_df.empty:
        st.warning("No recommendations data available for plotting.")
        return
...
def create_learner_recommendations_csv(recommendations_df):
    ...
    """
    if recommendations_df.empty:
        st.warning("No recommendations data available for CSV export.")
        return None
...
def plot_program_recommendations_heatmap(recommendations_df):
    ...
    """
    if recommendations_df.empty:
        st.warning("No recommendations data available for program analysis.")
        return
...
def create_program_recommendations_table(recommendations_df):
    ...
    program_counts_df = pd.DataFrame(learner_program_counts, columns=['parentKey', 'parentTitle'])
    program_counts_df = program_counts_df.groupby(['parentKey', 'parentTitle']).size().reset_index(name='usersRecommended')
    program_counts_df = program_counts_df.sort_values('usersRecommended', ascending=False)
    
    # Add top skills addressed (count unique users with gaps in each skill - only for users who are being recommended this as their top program)
    program_skills = []
    for _, row in program_counts_df.iterrows():
        program_key = row['parentKey']
        
        # Get users who have this program as their top recommendation
        top_program_users = set()
        for user_id in recommendations_df['userId'].unique():
            user_recommendations = recommendations_df[recommendations_df['userId'] == user_id]
            program_counts = user_recommendations['parentKey'].value_counts()
            if not program_counts.empty and program_counts.index[0] == program_key:
                top_program_users.add(user_id)
        
        # Get all weak skills for users who have this as their top program
        skill_user_counts = {}
        for user_id in top_program_users:
            user_recommendations = recommendations_df[recommendations_df['userId'] == user_id]
    ...
    program_counts_df['topSkillsAddressed'] = program_skills
    
    # Rename columns for display
    program_counts_df = program_counts_df.rename(columns={
        'parentKey': 'Key',
...
def get_attempts_dataframe(assessment_id):
    ...
    for attempt in raw_attempts:
        try:
            ...
            print(f"Skipping attempt due to error: {e}")
...
def plot_net_skills_heatmap(user_skills_df: pd.DataFrame):
    """
...
def plot_skill_correlation_heatmap(user_skills_df: pd.DataFrame):
    ...
    if len(skill_df) < 2:
        st.warning("Need at least 2 skills for correlation analysis.")
        return
...
def get_accessible_text_color(background_color):
    """
    Returns an accessible text color (black or white) based on background color brightness.
    """
...
def plot_question_analysis(results_df):
    ...
    if not question_stats:
        st.warning("No question statistics available for plotting.")
        return
    ...
    if len(stats_df) == 0:
        st.warning("No questions with sufficient data for analysis.")
        return
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"question_details_{assessment_id}.csv",
        mime="text/csv",
...
def plot_total_score_histogram(results_df):
    ...
    if len(all_scores) == 0:
        st.warning("No total score data available for histogram.")
        return
...
def plot_section_scores(results_df):
    ...
    if len(section_scores) == 0:
        st.warning("No section score data available for analysis.")
        return
