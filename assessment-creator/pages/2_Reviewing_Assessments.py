    ...
    with st.form(f"question_form_{question_index}"):
        ...
        if is_coding:
            ...
            edited_solution = st.text_area(
                ...
                height=120,
                help="Correct reference implementation that passes every test case. Must define a function named 'solution'."
            )
            # The harness is auto-generated by the API for non-SQL languages;
            # only SQL carries an author-provided DDL harness. Hiding it for
            # non-SQL prevents the 'solution' variable-not-defined misconfig.
            current_lang_enum = settings.CODING_LANGUAGE_OPTIONS.get(edited_language, edited_language)
            if str(current_lang_enum).upper() == "SQL":
                edited_harness = st.text_area(
                    "Test Harness Template (DDL; required for SQL)",
                    value=rec.get('test_harness_template') or '',
                    key=f"test_harness_{question_index}",
                    height=80,
                    help="DDL harness beginning with CREATE or WITH for SQL."
                )
            else:
                edited_harness = ''
                st.info(
                    f"Harness is auto-generated by the API for {edited_language}; not editable. "
                    "The candidate's function must be named `solution`."
                )
            ...
            # Validate / Dry-run: execute the edited solutionCode against the
            # edited test cases locally so staff catch misconfigurations before
            # upload. Mirrors the generator's pre-upload gate.
            if st.button("Validate / Dry-run", key=f"dryrun_{question_index}", help="Run solutionCode against the test cases locally."):
                try:
                    parsed_cases = json.loads(edited_test_cases) if edited_test_cases else []
                except json.JSONDecodeError as e:
                    st.error(f"Test cases are not valid JSON: {e}")
                    parsed_cases = None
                if parsed_cases is not None:
                    dryrun_details = {
                        'language': current_lang_enum,
                        'solutionCode': edited_solution,
                        'timeLimitMs': int(edited_time),
                        'testHarnessTemplate': edited_harness,
                    }
                    res = coding_dryrun.validate_and_run(dryrun_details, parsed_cases)
                    if res.get('contract_error'):
                        st.error(f"Contract failed: {res['contract_error']}")
                    elif res.get('skipped'):
                        st.warning(f"Dry-run skipped ({res.get('skip_reason')}). Contract OK.")
                    elif res.get('passed'):
                        st.success(f"Passed all {len(parsed_cases)} test case(s).")
                    else:
                        msgs = "; ".join(f"case {f['case_index']}: {f['reason']}" for f in res.get('failures', []))
                        st.error(f"Failed {len(res.get('failures', []))} test case(s): {msgs}")
